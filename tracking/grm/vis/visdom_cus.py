...
from grm.vis.plotting import overlay_mask, show_image_with_boxes
from grm.vis.utils import numpy_to_torch
...
class VisHeatmap(VisBase):
    ...
    def draw_data(self):
        if len(self.raw_data) == 2:
            self.visdom.heatmap(self.raw_data[0].clone(),
                                opts={'title': self.title + ' ' + self.raw_data[1]['caption'], **self.raw_data[1]},
                                win=self.title)
        else:
    ...
    def register(self, data, mode, debug_level=0, title='Data', **kwargs):
        if title not in self.registered_blocks.keys():
            ...
            if mode == 'image':
                ...
                raise ValueError('ERROR: unknown data mode {}'.format(mode))
        # Update
