        ...
        if self.manual_mode == False:
            if self.frame_count % 50 == 0 or self.single_track_mode == False or self.s_tracker == None:
                ...
                if drone_cty != None and len(dets) > 0:
                    if not check_tracker((drone_ctx, drone_cty), dets, thresh=10): # check_tracker return false
                        self.counter += 1
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Init Tracker
        self.tracker = Tracker(model_type='grm')
        ...
        self.selectSlicing.currentIndexChanged.connect(self.changeSizeWindow)
    ...
    def changeSizeWindow(self):
        if self.camera_dialog != None:
            self.camera_dialog.updateselectSlicing(self.selectSlicing.currentText())
    ...
    def open_camera_dialog(self, camera_index, ir=False):
        ...
        if ir:
            self.camera_dialog = CameraDialogIR(camera_index, detector=self.detector, tracker=self.tracker, 
                                         selectSlicing=self.selectSlicing.currentText(), updateList=self.updateList)
        else: 
            self.camera_dialog = CameraDialog(camera_index, detector=self.detector, tracker=self.tracker,
                                         selectSlicing=self.selectSlicing.currentText(), updateList=self.updateList)
